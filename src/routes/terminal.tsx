...
function TerminalPage() {
    ...
    useEffect(() => {
        GhosttyCore.load()
            ...
            .catch((err) => {
                ...
                setError(`Failed to load terminal core: ${err instanceof Error ? err.message : String(err)}`);
            })
    ...
    useEffect(() => {
        ...
        ws.onopen = () => {
            ...
            if (readyRef.current && ref.current?.instance) {
                sendResize(
                    ref.current.instance.cols,
                    ref.current.instance.rows,
                );
            }
            focusTerminal();
        };
    ...
    const handleError = useCallback((err: unknown) => {
        ...
        setError(
            `Terminal init failed: ${err instanceof Error ? err.message : String(err)}`,
        );
    }, []);
